...
exports.pastEvent = (req, res) => {
  upload.single("image")(req, res, (err) => {
    ...
    const sql =
      "UPDATE home SET pec_img = ?,  pec_title = ?, pec_des= ?, pec_mdes =? WHERE username = ?";
    pool.query(
      sql,
      [imagePath, title, description, text, username],
      (err, result) => {
        if (err) {
          console.error("Error inserting image path:", err);
          res.status(500).send("Error inserting image into the database.");
...
exports.upcomingEvent = (req, res) => {
  upload.single("image")(req, res, (err) => {
    ...
    const sql =
      "UPDATE home SET uec_img = ?,  uec_title = ?, uec_des= ?, uec_mdes =? WHERE username = ?";
    pool.query(
      sql,
      [imagePath, title, description, text, username],
      (err, result) => {
...
exports.futureEvent = (req, res) => {
  upload.single("image")(req, res, (err) => {
    ...
    const sql =
      "UPDATE home SET fec_img = ?,  fec_title = ?, fec_des= ?, fec_mdes =? WHERE username = ?";
    pool.query(
      sql,
      [imagePath, title, description, text, username],
      (err, result) => {
...
exports.updateContactUs = async (req, res) => {
  const { email, phoneOne, username } = req.body;
  try {
    const sql = "UPDATE home SET email = ?, phoneOne = ? WHERE username = ?";
    await pool.query(sql, [email, phoneOne, username], (err, result) => {
      if (err) {
        console.error("Error inserting text:", err);
        res.status(500).send("Error inserting text into the database.");
      } else {
